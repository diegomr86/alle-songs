  ...
  Alert.prototype.init = function() {
    var self;
    self = this;
    return $('#main-wrapper').on('click.pa.alerts', '.pa_page_alerts .close', function() {
  ...
  Alert.prototype.add = function(html, options) {
    ...
    if (!$box.length) {
      $box = $('<div id="' + options.container + '" />').prependTo($('#content-wrapper'));
    }
    $alerts = $('#' + options.container + ' .' + options.namespace);
    height = $alert.css({
      visibility: 'hidden',
      position: 'absolute',
      width: '100%'
